...
setup_xorg() {
    ...
    if -f "/etc/X11/xorg.conf.d/10-$kernel_driver"; then    # check if current VGA kernel driver's XORG conf exists.
...
find_line () {
    while IFS= read -r line; do
        ...
        if [[ ${$:0:22}='Kernel driver in use: ' ]]; then
            n=${#line}-21
            kernel_driver=${line:21:n}
            echo $kernel_driver # DEBUG
            if [[ $kernel_driver -ne "vfio-pci" ]]; then
                setup_xorg ()
            fi
        fi
    done
...
restart_dm () {
    cat /etc/X11/default-display-manager > $line
    echo $line  # DEBUG
    dm=${line:8:(${#line}-9)}
    echo $dm    # DEBUG
